...
tracker page in a right-side overlay inside LADXHD. The pause menu stays open underneath it, keeping
gameplay and music paused; closing the panel returns to that pause menu. The page is preconfigured
for Archipelago logic and connects to the game's WebSocket bridge through `127.0.0.1:17026`;
no address setup is required. Opening the panel starts the bridge on demand even when the profile
option below is disabled. The bridge implements
...
WebView storage keeps normal Magpie settings between visits. After the first load, closing the
overlay hides rather than destroys the WebView, so its page, tracker state, and local connection
continue running in the background until the game activity exits. The toolbar's Reload button is
available when an explicit page refresh is needed.
