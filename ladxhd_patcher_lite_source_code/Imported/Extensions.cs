...
namespace LADXHD_Patcher_Lite
{
    ...
    internal static class Extensions
    {
        ...
        public static void RemovePath(this string inputPath, bool throwOnFailure = false)
        {
            // If the path is empty then it does not exist.
            ...
            // Retries exhausted: the delete genuinely failed (locked file, etc.).
            if (throwOnFailure)
            {
                throw new PathOperationException($"Could not delete after multiple attempts:\n{inputPath}\n\n{lastError?.Message}");
            }
        }

        public static void MovePath(this string Source, string Destination, bool Overwrite = false, bool throwOnFailure = false)
        {
            // If the values are null or empty then return false.
            if (string.IsNullOrWhiteSpace(Source) || string.IsNullOrWhiteSpace(Destination))
            {
                if (throwOnFailure)
                    throw new PathOperationException("Move failed: source or destination path was empty.");
                return;
            }
            ...
            {
                if (throwOnFailure)
                    throw new PathOperationException($"Move failed: source does not exist:\n{Source}");
                return;
            }
