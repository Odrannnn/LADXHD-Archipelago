...
namespace ProjectZ.Base
{
    ...
    public class InputHandler : GameComponent
    {
        ...
        public static bool WantsTextInput => _textInputEnabled;
        ...
        private static KeyboardState StripFullscreenChord(KeyboardState state)
        {
            // Alt+Enter is reserved for the fullscreen toggle. While Alt is held,
            // remove Enter from the keyboard state entirely so no part of the game
            // (query methods or raw KeyboardState reads) ever sees it as input.

            bool altDown = state.IsKeyDown(Keys.LeftAlt) || state.IsKeyDown(Keys.RightAlt);
            if (!altDown || state.IsKeyUp(Keys.Enter))
                return state;

            var down = state.GetPressedKeys();
            var filtered = new List<Keys>(down.Length);
            for (int i = 0; i < down.Length; i++)
                if (down[i] != Keys.Enter)
                    filtered.Add(down[i]);

            return new KeyboardState(filtered.ToArray(), state.CapsLock, state.NumLock);
        }

        public override void Update(GameTime gameTime)
        {
            _lastKeyboardState = _keyboardState;
            _keyboardState = StripFullscreenChord(Keyboard.GetState());

            _lastMouseState = _mouseState;
            _mouseState = Mouse.GetState();

            Game1.PlatformInput.BeginFrame();
            ...
            // Prevents input when Window is in the background.
            if (!Game1.WasActive)
                ResetInputState();
        }
        ...
        {
            return Game1.PlatformInput.ConsumeSelectPressed();
        }
        ...
        public static void EnableTextInput()
        {
            ...
            _textInputBuffer = "";
            Game1.TextInputService.SetEnabled(true);
        }
        ...
        public static void DisableTextInput()
        {
            ...
            _textInputBuffer = "";
            Game1.TextInputService.SetEnabled(false);
        }

        public static void ReceiveTextInput(char character)
        {
