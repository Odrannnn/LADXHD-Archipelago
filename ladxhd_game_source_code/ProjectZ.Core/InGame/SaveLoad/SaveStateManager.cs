        ...
        public static void LoadSaveData()
        {
            for (var i = 0; i < SaveCount; i++)
            {
                ...
                if (saveManager.LoadFile(Path.Combine(Values.PathSaveFolder, SaveGameSaveLoad.SaveFileName + i)))
                {
                    ...
                    // The file-select summary intentionally contains only display data. Read the
                    // full save as well so the AP badge represents the save's actual binding.
                    var gameSaveManager = new SaveManager();
                    if (gameSaveManager.LoadFile(Path.Combine(Values.PathSaveFolder,
                            SaveGameSaveLoad.SaveFileNameGame + i)))
                    {
                        SaveStates[i].ArchipelagoBound =
                            !string.IsNullOrWhiteSpace(gameSaveManager.GetString("ap_seed_name")) &&
                            !string.IsNullOrWhiteSpace(gameSaveManager.GetString("ap_slot_name"));
                    }
