...
namespace ProjectZ.InGame.GameObjects.Enemies
{
    internal class EnemyShroudedStalfos : GameObject
    {
        ...
        public EnemyShroudedStalfos(Map.Map map, int posX, int posY) : base(map)
        {
            ...
            var hittabelBox = new CBox(EntityPosition, -7, -15, 0, 14, 15, 8);
            var pushableBox = new CBox(EntityPosition, -7, -11, 0, 14, 11, 4);

            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageBox, HitType.Enemy, 2));
            AddComponent(HittableComponent.Index, _hitComponent = new HittableComponent(hittabelBox, OnHit) { ArrowMultiplier = true, BombMultiplier = true, BoomerangMultiplier = true });
