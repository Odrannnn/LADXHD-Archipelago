...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossGenie : GameObject, IHasVisibility
    {
        ...
        public BossGenie(Map.Map map, string saveKey, Vector3 position, BossGenieBottle objBottle) : base(map)
        {
            ...
            AddComponent(PushableComponent.Index, _pushableComponent = new PushableComponent(damageCollider, OnPush) { IsActive = false, RepelMultiplier = 2.05f });
            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageCollider, HitType.Enemy, 4) { IsActive = false, OnDamagedPlayer = OnDamagedPlayer });
            AddComponent(HittableComponent.Index, _hitComponent = new HittableComponent(hittableBox, OnHit) { BombMultiplier = true, MagicRodMultiplier = true });
        ...
        private void RotateEnd()
        {
            ...
            // throw fireball
            var fireball = new BossGenieFireball(Map, this, EntityPosition.ToVector3(), genie_fireball_shader);
        ...
        private void InitAttack()
        {
            ...
            for (var i = 0; i < _fireballCount; i++)
            {
                ...
                _fireballs[i] = new BossGenieFireball(Map, this, new Vector3(EntityPosition.X, EntityPosition.Y - 1, EntityPosition.Z + 12), genie_fireball_shader);
        ...
        private void OnDeath()
        {
            ...
            // spawn big heart
            Map.Objects.SpawnObject(new ObjItem(Map, (int)heartPosition.X - 8, (int)heartPosition.Y - 16, "j", "d2_nHeart", "heartMeterFull", null));

            // Achievement: If the player didn't take damage, unlock the achievement.
            if (!DealtPlayerDamage)
                AchievementManager.Earn(22);
        ...
        private bool OnPush(Vector2 direction, PushableComponent.PushType type)
        {
            ...
            _body.Velocity.X = direction.X * 3.00f;
            _body.Velocity.Y = direction.Y * 3.00f;
