...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossSlimeEel : GameObject
    {
        ...
        public BossSlimeEel(Map.Map map, Vector2 centerPosition, BossSlimeEelSpawn eelSpawner, string saveKey) : base(map)
        {
            ...
            {
                if (i == 0)
                    _tailParts[i] = new BossSlimeEelTail(Map, _eelSpawner, EntityPosition.Position, 0, OnHitHeart);
                else
                    _tailParts[i] = new BossSlimeEelTail(Map, _eelSpawner, EntityPosition.Position, 0, null);
            ...
            stateBlink.Trigger.Add(new AiTriggerCountdown(AiDamageState.StaticCooldown, TickBlink, Explode));
            ...
            var hittableBox = new CBox(EntityPosition, -9, -9, 0, 18, 18, 8);
            ...
            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageCollider, HitType.Enemy, 6) { IsActive = false, OnDamagedPlayer = OnDamagedPlayer });
        }

        private void OnDamagedPlayer()
        {
            // If it deals damage transfer to the main boss.
            _eelSpawner.DealtPlayerDamage = true;
        }
        ...
        private void UpdateExplode()
        {
            ...
            if (collisionRect != Rectangle.Empty)
            {
                ...
                if (collisionBox.Intersects(MapManager.ObjLink.Body.BodyBox.Box))
                    MapManager.ObjLink.HitPlayer(collisionBox, HitType.Bomb, 6);
            }
        ...
        {
            _sprite.SpriteShader = (AiDamageState.StaticCooldown - counter) % (AiDamageState.BlinkTime * 2) <
        ...
        private void OnDeath()
        {
            ...
            // If no damage was taken then grant the achievement.
            if (!_eelSpawner.DealtPlayerDamage)
                AchievementManager.Earn(69);
        }
        ...
        public Values.HitCollision OnHitHeart(GameObject originObject, Vector2 direction, HitType hitType, int damage, bool pieceOfPower)
        {
            ...
            // Slime Eel works on a "hit" basis and takes 8 hits.
            damage = 1;
