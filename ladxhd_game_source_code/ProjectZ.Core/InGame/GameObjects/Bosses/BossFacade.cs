...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossFacade : GameObject
    {
        ...
        public BossFacade(Map.Map map, int posX, int posY, string saveKey, string saveKeyHeart) : base(map)
        {
            ...
            statePreDeath.Trigger.Add(new AiTriggerCountdown(100, null, () => _damageState.OnDeathBoss(false)));
            ...
            _damageState = new AiDamageState(this, null, _aiComponent, _sprite, _lives, 0, false, false, AiDamageState.BlinkTime * 8) 
            {
                MoveBody = false,
                HitMultiplierX = 0,
                HitMultiplierY = 0,
                ...
                ExplosionOffsetY = 32,
                OnLiveZeroed = StartDeathSequence,
            };
            _damageState.AddBossDamageState(DeathAnimationEnd);
            _damageState.OnDeath = (pieceOfPower) => { };

            AddComponent(AiComponent.Index, _aiComponent);
            AddComponent(HittableComponent.Index, new HittableComponent(hittableRectangle, OnHit) { BombMultiplier = true });
            AddComponent(DrawComponent.Index, new DrawComponent(Draw, Values.LayerBottom, EntityPosition));
            AddComponent(KeyChangeListenerComponent.Index, new KeyChangeListenerComponent(OnKeyChange));
            AddComponent(UpdateComponent.Index, new UpdateComponent(UpdateFall));
        ...
        {
            if (MapManager.ObjLink.Body.IsAbsorbed)
        ...
        private void SpawnTilesAndPots()
        {
            for (var i = 0; i < _potPositions.Length; i++)
            {
                ...
                _objPots[i].OnDamagedPlayer = () => DealtPlayerDamage = true;
            ...
            for (var i = 0; i < _tilePositions.Length; i++)
            {
                ...
                flyingTile.OnHitPlayer = () => DealtPlayerDamage = true;
        ...
        private void UpdateInit()
        {
            ...
            {
                Game1.AudioManager.SetMusicFadeTransition(24, 2, 350);
        ...
        private void Draw(SpriteBatch spriteBatch)
        {
            ...
            // Read the damage blink from the CSprite, where AiDamageState writes it.
            var damageShader = _sprite.SpriteShader;

            if (damageShader != null)
            {
                spriteBatch.End();
                ObjectManager.SpriteBatchBegin(spriteBatch, damageShader);
            }
        ...
        {
            // Facade is only vulnerable to bombs, and only while active and visible.
            if (!_hittable || !IsVisible() || (hitType & HitType.Bomb) == 0)
                return Values.HitCollision.None;

            // Still inside the hit cooldown window (replaces the old _blinkTigger.IsRunning() gate).
            if (_damageState.DamageTrigger.CurrentTime > 0)
                return Values.HitCollision.None;

            var hitCollision = _damageState.OnHit(gameObject, direction, hitType, damage, false);

            // Only schedule the despawn if the damage state actually registered the hit.
        ...
        private void StartDeathSequence()
        {
            ...
            Game1.AudioManager.SetMusicFadeTransition(93, 2, 350);
        ...
        private void DeathAnimationEnd()
        {
            ...
            // Earn the achievement if the player did not take damage.
            if (!DealtPlayerDamage)
                AchievementManager.Earn(79);
