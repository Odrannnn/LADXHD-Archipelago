...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossGenieBottle : GameObject
    {
        ...
        private AiTriggerCountdown _triggerCountdown;
        ...
        public BossGenieBottle(Map.Map map, int posX, int posY, string saveKey) : base(map)
        {
            ...
            stateShaking.Trigger.Add(_triggerCountdown = new AiTriggerCountdown(600, TickShake, ShakeEnd));
            ...
            _damageState = new AiDamageState(this, _body, _aiComponent, sprite, _lives, 0) { MoveBody = false, OnDeath = OnDeath, BossHitSound = true };

            _aiComponent.ChangeState("idle");

            AddComponent(PushableComponent.Index, new PushableComponent(pushableBox, OnPush));
            AddComponent(AiComponent.Index, _aiComponent);
            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageCollider, HitType.Enemy, 4) { OnDamagedPlayer = OnDamagedPlayer, IsActive = false });
            AddComponent(HittableComponent.Index, new HittableComponent(hittableBox, OnHit));
            AddComponent(CarriableComponent.Index, _carriableComponent = new CarriableComponent(carryRectangle, CarryInit, CarryUpdate, CarryThrow) { IsInstant = true, StartGrabbing = StartGrabbing, IsActive = false });
        ...
        private void Update()
        {
            // Don't bounce Link when following or when bottle is not in wobble.
            if (_animator.AnimationID != "wobble" || _aiComponent.CurrentStateId == "follow")
                return;

            // Get Link and create a box around the bottle.
            var link = MapManager.ObjLink;
            var bottleRect = new Box((int)EntityPosition.X - 10, (int)EntityPosition.Y - 18, 0, 20, 20, 8);

            // When the boxes intersect.
            if (bottleRect.Intersects(link.Body.BodyBox.Box))
            {
                // Calculate the knockback direction.
                var direction = link.Position - new Vector2(EntityPosition.X, EntityPosition.Y - 8);

                // Normalize the direction.
                if (direction != Vector2.Zero)
                    direction.Normalize();

                // Apply the knockback to Link through shield velocity.
                link.ShieldVelocity = new Vector2(direction.X, direction.Y) * 2.65f;

                // Play the "bump" sound effect.
                if (!Game1.AudioManager.IsPlaying("D360-09-09"))
        ...
        private void TickShake(double time)
        {
            ...
            if (_triggerCountdown.CurrentTime < 100)
                _carriableComponent.IsActive = false;
        }
        ...
        {
            if (_aiComponent.CurrentStateId == "thrown" && (collision & (Values.BodyCollision.Horizontal | Values.BodyCollision.Vertical)) != 0)
            {
                _aiComponent.ChangeState("return");
        ...
        {
            if ((hitType & HitType.AnySword) == 0 && (hitType & HitType.Bomb) == 0)
