...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossSlimeEyeHalf : GameObject
    {
        ...
        public BossSlimeEyeHalf(Map.Map map, Vector2 position, string animationName, string saveKey) : base(map)
        {
            ...
            _damageState = new AiDamageState(this, _body, _aiComponent, _sprite, _lives, 0, false) { BossHitSound = true };
            ...
            AddComponent(DamageFieldComponent.Index, _damageComponent = new DamageFieldComponent(damageCollider, HitType.Enemy, 4) { OnDamagedPlayer = OnDamagedPlayer });
            AddComponent(HittableComponent.Index, _hitComponent = new HittableComponent(hittableRectangle, OnHit) { MagicRodMultiplier = true });
        ...
        private void OnDamagedPlayer()
        {
            if (!string.IsNullOrEmpty(_saveKey))
                Game1.GameManager.SaveManager.SetString(_saveKey + "dmg", "1");
        }
        ...
        private void OnDeath()
        {
            if (!string.IsNullOrEmpty(_saveKey))
            {
                ...
                else
                {
                    ...
                    // Spawn big heart.
                    Map.Objects.SpawnObject(new ObjItem(Map, (int)EntityPosition.X - 8, (int)EntityPosition.Y - 24, "j", "d3_nHeart", "heartMeterFull", null));

                    // If the player did not take any damage earn the achievement.
                    if (Game1.GameManager.SaveManager.GetString(_saveKey + "dmg", "0") != "1")
                        AchievementManager.Earn(37);
                }
            }
        ...
        private Values.HitCollision OnHit(GameObject gameObject, Vector2 direction, HitType hitType, int damage, bool pieceOfPower)
        {
            ...
            if ((hitType & HitType.Sword) == 0 || _damageState.IsInDamageState())
