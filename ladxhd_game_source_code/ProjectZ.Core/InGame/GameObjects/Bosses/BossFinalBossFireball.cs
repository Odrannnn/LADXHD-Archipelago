...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossFinalBossFireball : GameObject
    {
        ...
        private const int ChargeStateTime = 1100;
        private const int ChargeStateCount = 3;
        ...
        private int   light_size   = 50;

        // Not configurable. Don't expose in lahdmod.
        private float _lightFade   = 1f;

        public BossFinalBossFireball(BossFinalBoss owner, Vector2 position) : base(owner.Map)
        {
            // If a mod file exists load the values from it.
            string modFile = Path.Combine(Values.PathLAHDMods, "BossFinalBossFireball.lahdmod");
            ...
            state0.Trigger.Add(new AiTriggerCountdown(ChargeStateTime, c => TickCharge(0, c),
                () => _aiComponent.ChangeState("idle1")));

            var state1 = new AiState() { Init = () => _animator.Play("idle_1") };
            state1.Trigger.Add(new AiTriggerCountdown(ChargeStateTime, c => TickCharge(1, c),
                () => _aiComponent.ChangeState("idle2")));

            var state2 = new AiState() { Init = () => _animator.Play("idle_2") };
            state2.Trigger.Add(new AiTriggerCountdown(ChargeStateTime, c => TickCharge(2, c),
                () => { _lightIntensity = light_size; _isReady = true; }));
        ...
        private void Update()
        {
            if (_isMoving)
            {
                ...
                if (_isRepelled && _owner.HitBoss(this, _body.VelocityTarget * 0.75f, _damageBox))
                    _moveTime = 1300;
                ...
                if (_moveTime > 1300)
                {
                    ...
                    _lightFade = MathHelper.Clamp(1 - ((float)_moveTime - 1300) / 100, 0, 1);
                }
        ...
        private void Repell()
        {
            ...
            if (_objTrail0 != null)
                _objTrail0.Sprite.Color = Color.White;
            if (_objTrail1 != null)
                _objTrail1.Sprite.Color = Color.White;
        ...
        private void OnDeath(bool PlaySound = false)
        {
            ...
            var objFireball0 = new EnemyFireball(Map, posX - 2, posY - 2, 1, true, 75);
            var objFireball1 = new EnemyFireball(Map, posX + 2, posY - 2, 1, true, 75);
            var objFireball2 = new EnemyFireball(Map, posX - 2, posY + 2, 1, true, 75);
            var objFireball3 = new EnemyFireball(Map, posX + 2, posY + 2, 1, true, 75);
        ...
        private void TickCharge(int stateIndex, double counter)
        {
            // Track the state of "AiTriggerCountdowns" to increase the light size over time.
            var elapsed = stateIndex * ChargeStateTime + (ChargeStateTime - counter);
            var progress = MathHelper.Clamp((float)(elapsed / (ChargeStateTime * ChargeStateCount)), 0f, 1f);
            _lightIntensity = light_size * progress;
        }

        private void DrawLight(SpriteBatch spriteBatch)
        {
            if (!GameSettings.ObjectLights || !light_source || _lightIntensity <= 0)
                return;

            var lightColor = new Color(light_red, light_grn, light_blu) * light_bright * _lightFade;

            DrawLightAt(spriteBatch, lightColor, _body.Position.X, _body.Position.Y - _body.Position.Z, _lightIntensity);

            if (_objTrail0 != null)
                DrawLightAt(spriteBatch, lightColor, _objTrail0.EntityPosition.X, _objTrail0.EntityPosition.Y, _lightIntensity * 0.85f);

            if (_objTrail1 != null)
                DrawLightAt(spriteBatch, lightColor, _objTrail1.EntityPosition.X, _objTrail1.EntityPosition.Y, _lightIntensity * 0.70f);
        }

        private void DrawLightAt(SpriteBatch spriteBatch, Color lightColor, float centerX, float centerY, float size)
        {
            if (size < 1f)
                return;
