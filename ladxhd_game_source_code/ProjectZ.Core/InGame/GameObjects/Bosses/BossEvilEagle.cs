...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class BossEvilEagle : GameObject
    {
        ...
        public BossEvilEagle(Map.Map map, int posX, int posY, string saveKey) : base(map)
        {
            ...
            _damageState = new AiDamageState(this, _body, _aiComponent, _sprite, _lives, 0, false, false, AiDamageState.BlinkTime * 2 * 10) { ExplosionOffsetY = 8, PlayDeathSound = true };
            _damageState.AddBossDamageState(OnDeath);

            AddComponent(AiComponent.Index, _aiComponent);
            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageCollider, HitType.Enemy, 8) { OnDamagedPlayer = OnDamagedPlayer });
            AddComponent(HittableComponent.Index, _hittableComponent = new HittableComponent(hittableRectangle, OnHit) { BombMultiplier = true, MagicRodMultiplier = true });
        ...
        private void GenericUpdate()
        {
            if (_disablePlayer)
            {
                ...
                MapManager.ObjLink.Body.Velocity = Vector3.Zero;
            }
        }
        ...
        private void InitStartSequence()
        {
            ...
            _creeperFly0 = new MBossGrimCreeperFly(Map, _grimCreeper, new Vector2(EntityPosition.X - 41, EntityPosition.Y + 32), new Vector2(-24, 12));
            _creeperFly0.StartSequenceMode();
            Map.Objects.SpawnObject(_creeperFly0);

            _creeperFly1 = new MBossGrimCreeperFly(Map, _grimCreeper, new Vector2(EntityPosition.X - 7, EntityPosition.Y + 32), new Vector2(-16, -8));
        ...
        private void UpdateWingAttack()
        {
            ...
            if (MapManager.ObjLink.EntityPosition.Y < _startPosition.Y + 90)
                MapManager.ObjLink.Body.AdditionalMovementVT.X = _direction * 1.1f;
            else
                MapManager.ObjLink.Body.AdditionalMovementVT.X = 0;
            ...
            if (_featherCounter > 275)
            {
                ...
                var eagleFeather = new BossEvilEagleFeather(Map, this, startPosition, aimDirection * 4);
