...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    internal class BossHotHead : GameObject
    {
        ...
        public BossHotHead(Map.Map map, int posX, int posY, string saveKey) : base(map)
        {
            ...
            _aiComponent.Trigger.Add(new AiTriggerUpdate(CheckLavaDamage));
            ...
            _damageState = new AiDamageState(this, _body, _aiComponent, _sprite, _lives, 0, false, false)
            {
                HitMultiplierX = 0,
                HitMultiplierY = 0,
                ExplosionOffsetY = 4,
            ...
            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageCollider, HitType.Enemy, 16) { OnDamagedPlayer = OnDamagedPlayer });
            AddComponent(HittableComponent.Index, _hitComponent = new HittableComponent(hittableBox, OnHit) { MagicRodMultiplier = true });
        ...
        {
            if (MapManager.ObjLink.DrownedInLava())
                DealtPlayerDamage = true;
        }
        ...
        {
            var speedMult = 0.65f;
            Map.Objects.SpawnObject(new BossHotHeadSplash(Map, this, new Vector2(EntityPosition.X - 8, EntityPosition.Y - 8), new Vector2(-1, -1) * speedMult));
            Map.Objects.SpawnObject(new BossHotHeadSplash(Map, this, new Vector2(EntityPosition.X - 8, EntityPosition.Y), new Vector2(-1, 1) * speedMult));
            Map.Objects.SpawnObject(new BossHotHeadSplash(Map, this, new Vector2(EntityPosition.X + 8, EntityPosition.Y - 8), new Vector2(1, -1) * speedMult));
            Map.Objects.SpawnObject(new BossHotHeadSplash(Map, this, new Vector2(EntityPosition.X + 8, EntityPosition.Y), new Vector2(1, 1) * speedMult));
        }
        ...
        public Values.HitCollision OnHit(GameObject gameObject, Vector2 direction, HitType hitType, int damage, bool pieceOfPower)
        {
            ...
            if (_aiComponent.CurrentStateId == "breaking" || _aiComponent.CurrentStateId == "broken" || _damageState.IsInDamageState())
            ...
            if (_aiComponent.CurrentStateId == "moving" || _aiComponent.CurrentStateId == "damaged")
            {
                ...
                if (!_damaged && _damageState.CurrentLives <= 8)
                {
