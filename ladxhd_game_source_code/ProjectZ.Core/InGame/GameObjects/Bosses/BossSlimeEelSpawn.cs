    ...
    class BossSlimeEelSpawn : GameObject
    {
        ...
        public BossSlimeEelSpawn(Map.Map map, int posX, int posY, string saveKey) : base(map)
        {
            ...
            for (var i = 0; i < _tailParts.Length; i++)
            {
                ...
                _tailParts[i] = new BossSlimeEelTail(map, this, _centerPosition, spriteIndex, null);
        ...
        private void SpawnHoles()
        {
            for (var i = 0; i < 4; i++)
            {
                var hole = new ObjHole(Map, (int)_centerPosition.X - 16 + (i % 2) * 16, (int)_centerPosition.Y - 16 + (i / 2) * 16, 16, 16, Rectangle.Empty, 0, 0, 0);
