    ...
    class BossSlimeEye : GameObject
    {
        ...
        public BossSlimeEye(Map.Map map, int posX, int posY, string enterKey, string triggerKey, string saveKey) : base(map)
        {
            ...
            if (!string.IsNullOrEmpty(_saveKey))
                Game1.GameManager.SaveManager.SetString(_saveKey + "dmg", "0");
            ...
            _damageState = new AiDamageState(this, _body, _aiComponent, _sprite, _lives, 0, false);
        ...
        private void TickSpawn()
        {
            for (var i = 0; i < _zols.Length; i++)
            {
                if (_zols[i] == null || _zols[i].Map == null)
                {
                    ...
                    _zols[i] = new EnemyGreenZol(Map, (int)position.X, (int)position.Y, 100, true, this);

                    if (!string.IsNullOrEmpty(_saveKey))
                    {
                        var dmgKey = _saveKey + "dmg";
                        _zols[i].OnHitPlayer = () => Game1.GameManager.SaveManager.SetString(dmgKey, "1");
                    }
