...
namespace ProjectZ.InGame.GameObjects.Things
{
    public class ObjRaft : GameObject
    {
        ...
        public void Jump(Vector2 targetPosition, int time)
        {
            ...
            MapManager.ObjLink.Body.IsGrounded = false;
            MapManager.ObjLink.Body.IsActive = false;
        }
        ...
        private void UpdateMoving()
        {
            ...
            if (_jumpTime > 0)
            {
                ...
                else
                {
                    ...
                    {
                        MapManager.ObjLink.Body.IsActive = true;
                        Body.IsActive = true;
                    }
        ...
        public void TrackRupees(int amount)
        {
            // Add the amount of rupees that were collected.
            RupeesCollected += amount;

            // If the player manages to get 100 in a single ride grant the achievement.
            if (RupeesCollected >= 100 && !AchievementManager.IsEarned(74))
                AchievementManager.Earn(74);
        }
    }
}