...
namespace ProjectZ.InGame.GameObjects.Things
{
    public class ObjFinalBackground : GameObject
    {
        ...
        private static Vector2 CameraGamePosition => GameSettings.PixelSnapping
            ? new Vector2(MapManager.Camera.RoundX, MapManager.Camera.RoundY) / MapManager.Camera.Scale
            : new Vector2(MapManager.Camera.X, MapManager.Camera.Y) / MapManager.Camera.Scale;

        private static Vector2 SnapToGrid(Vector2 position) => GameSettings.PixelSnapping
            ? new Vector2(MathF.Round(position.X), MathF.Round(position.Y))
            : position;

        private static Vector2 SnapShaderOffset(Vector2 screenOffset)
        {
            if (!GameSettings.PixelSnapping)
                return screenOffset;

            var scale = MapManager.Camera.Scale;
            var phase = MapManager.Camera.PixelGridOffset * scale;

            return new Vector2(
                MathF.Round((screenOffset.X - phase.X) / scale) * scale + phase.X,
                MathF.Round((screenOffset.Y - phase.Y) / scale) * scale + phase.Y);
        }
        ...
        {
            // Offset the background objects relative to the camera so that they move slower than the camera pans.
            var starOffset = -(_spawnPosition.Position - CameraGamePosition) * new Vector2(1.0f, 0.75f);
            foreach (var star in _stars)
            {
                ...
                star.animator.Draw(spriteBatch, SnapToGrid(offsetPosition + starOffset), star.color);
            }
            // The shader paints the dithered gradient from SV_Position, so the pattern phase comes from "offset"
            // and not from the snapped sprite position. "Location" is the raw unsnapped camera and the 0.85 parallax
            // turns it fractional, which left the dither blocks sitting part way inside a game pixel while the cloud
            // outline was already aligned. Use the rounded camera value the transform matrix uses, then quantize.
            var camVecX = MapManager.Camera.RoundX + 4000;
            var camVecY = MapManager.Camera.RoundY;
            var camVector = new Vector2(camVecX, camVecY) * 0.85f;
            var renVector = new Vector2(Game1.RenderWidth, Game1.RenderHeight) / 2;
            var shaderOffset = SnapShaderOffset(camVector - renVector);
            ...
            foreach (var cloud in _clouds)
            {
                ...
                var offset = -(_spawnPosition.Position - CameraGamePosition) * new Vector2(1.0f, 0.35f) * cloud.offset;
