...
{
    internal class ObjStone : GameObject, IHasSpriteVisibility
    {
        ...
        // Properties shared with "BossFacade".
        public Action OnDamagedPlayer;

        // Properties shared with ObjObjectRespawner.
        ...
        public ObjStone(Map.Map map, int posX, int posY, string spriteId, string spawnItem, string pickupKey, string dialogPath, bool isHeavy, bool potMessage) : base(map, spriteId)
        {
            ...
            _isPot2D = spriteId == "pot_2";
            _isSkull = spriteId == "skull";
            ...
            var collisionTypes = _isHeavy ? Values.CollisionTypes.Normal : Values.CollisionTypes.Normal | Values.CollisionTypes.Hookshot;

            AddComponent(BodyComponent.Index, _body);
            AddComponent(CarriableComponent.Index, _carriableComponent = new CarriableComponent(carryRect, CarryInit, CarryUpdate, CarryThrow) { IsHeavy = _isHeavy, StartGrabbing = StartGrabbing });
            AddComponent(CollisionComponent.Index, _collisionComponent = new BoxCollisionComponent(collisionBox, collisionTypes));
        ...
        private void Update()
        {
            ...
            // The amount of damage dealt when hitting an enemy with a stone.
            var damage = 2;

            // Find the right hittype with the correct amount of damage or create a extra one?
            var hitCollision = Map.Objects.Hit(this, _damageBox.Box.Center, _damageBox.Box, HitType.ThrownObject, damage, false);
        ...
        public void OnCollision()
        {
            ...
            // Track if a stone is destroyed carried by a beetle.
            if (OnSpinyBeetle)
            ...
            // Stones and skulls have a 20% chance to spawn a fairy and must have come from a spiny beetle.
            if (OnSpinyBeetle && Game1.RandomNumber.Next(0,5) == 0)
            {
                var objFairy = new ObjDungeonFairy(Map, (int)EntityPosition.X, (int)EntityPosition.Y, 0);
                Map.Objects.SpawnObject(objFairy);
            }
        ...
        {
            if (OnSpinyBeetle && (hitType & HitType.Boomerang) != 0)
                return Values.HitCollision.Blocking;            
            
            else if (_isSkull && OnSpinyBeetle && GameSettings.SwBreakPots && (hitType & HitType.SwordShot) != 0 && Game1.GameManager.GetItem("sword2") != null)
            {
                OnCollision();
                SpawnItem();
                IsDead = true;
            ...
            else if (GameSettings.SwBreakPots && 
