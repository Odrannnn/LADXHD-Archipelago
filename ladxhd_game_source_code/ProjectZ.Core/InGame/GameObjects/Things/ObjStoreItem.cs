...
namespace ProjectZ.InGame.GameObjects.Things
{
    internal class ObjStoreItem : GameObject
    {
        ...
        public ObjStoreItem(Map.Map map, int posX, int posY, string itemName, int itemPrice, int count) : base(map)
        {
            ...
            _itemName = itemName;
            var sourceLocationKey = ArchipelagoLocationKey.Shop(itemPrice);
            var displayItemName = Game1.GameManager.ArchipelagoManager.ResolveLocationItemName(
                sourceLocationKey, itemName, map?.MapName, posX, posY);
            _item = Game1.GameManager.ItemManager[displayItemName];

            bool shopPunish = Game1.GameManager.SaveManager.GetString("stoleItem", "0") == "1";

            if (_item == null || shopPunish ||
                Game1.GameManager.ArchipelagoManager.IsLocationCheckComplete(sourceLocationKey))
            {
                IsDead = true;
                return;
            }
        ...
        private void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (_itemCount > 1)
            {
                DrawHelper.DrawString(spriteBatch, "x", new Vector2((int)(_itemPosition.X + _sourceRectangle.Width), (int)(EntityPosition.Y + 24 - ItemDrawHelper.LetterHeight)), Color.Black);
