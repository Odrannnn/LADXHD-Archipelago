...
namespace ProjectZ.InGame.GameObjects.Things
{
    public class ObjDoorEgg : GameObject
    {
        ...
        public ObjDoorEgg(Map.Map map, int posX, int posY, string saveId) : base(map)
        {
            ...
            AddComponent(CarriableComponent.Index, new CarriableComponent(new CRectangle(EntityPosition, new Rectangle(-7, -15, 14, 14)), null, null, null) { IsCollision = true });
            AddComponent(DrawComponent.Index, new DrawComponent(Draw, Values.LayerBottom, EntityPosition));
        }

        private void OnSongPlayed(int songIndex)
        {
            // Make sure it's the correct song.
            ...
            _shownInstrument = 0;
            _drawInstruments = true;
            _shakeScreen = false;
            for (var i = 0; i < 8; i++)
                _showInstrument[i] = false;

            // Get the instruments the player has.
            for (var i = 0; i < 8; i++)
            {
