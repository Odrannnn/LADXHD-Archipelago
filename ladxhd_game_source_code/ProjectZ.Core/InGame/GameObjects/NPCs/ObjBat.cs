...
namespace ProjectZ.InGame.GameObjects.NPCs
{
    internal class ObjBat : GameObject
    {
        ...
        {
            var eventKey = Map?.MapName switch
            {
                "caveBat1.map" => "mad_batter_forest",
                "caveBat2.map" => "mad_batter_mountain",
                "caveBat3.map" => "mad_batter_bay",
                _ => null
            };
            if (eventKey != null)
            {
                var sourceLocationKey = ArchipelagoLocationKey.Event(eventKey);
                Game1.GameManager.ArchipelagoManager.ResolveLocationItemName(
                    sourceLocationKey, "powder", Map?.MapName,
                    (int)EntityPosition.X, (int)EntityPosition.Y);
                if (Game1.GameManager.ArchipelagoManager.TryHandleLocationCheck(
                    new GameItemCollected("powder")
                    {
                        Count = 1,
