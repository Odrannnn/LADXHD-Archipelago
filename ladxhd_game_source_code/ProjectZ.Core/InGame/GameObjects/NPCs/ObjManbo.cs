...
namespace ProjectZ.InGame.GameObjects.NPCs
{
    internal class ObjManbo : GameObject
    {
        ...
        private void Update()
        {
            ...
            if (_isPlaying)
            {
                // Disable the inventory during the song.
                Game1.GameManager.InGameOverlay.DisableInventoryToggle = true;

                // We'll be referncing Link a bit so lets be efficient.
                var Link = MapManager.ObjLink;
                var _linkInWater = (Link.Body.CurrentFieldState & MapStates.FieldStates.DeepWater) != 0;

                // Freeze the player during the song.
                Link.FreezePlayer();

                // Do some basic swimming detection. This will enter swimming state, but Link will fall.
                if (_linkInWater && Link.CurrentState != ObjLink.State.PickingUp)
                    Link.CurrentState = ObjLink.State.Swimming;

                // Once he's positioned in front of the fish, run the ObjLink2d swimming function.
                if (Link.Position.Y >= 86)
                    Link.Update2DSwimming();
