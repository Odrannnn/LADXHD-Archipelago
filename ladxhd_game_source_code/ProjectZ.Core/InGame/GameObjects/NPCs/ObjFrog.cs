...
namespace ProjectZ.InGame.GameObjects.NPCs
{
    internal class ObjFrog : GameObject
    {
        ...
        private Vector3 _storeVelocity;
        ...
        {
            // Textboxes can't pause everyting in Richard's Villa or the achievement
            // timer will fall out of sync. So we can simulate pausing with animation freeze.
            if (Game1.GameManager.InGameOverlay.TextboxOverlay.IsOpen)
            {
                if (!_textboxSimulated)
                {
                    _storeVelocity = (_body.Velocity != Vector3.Zero) ? _body.Velocity : Vector3.Zero;
                    _animator.Pause();
                    _body.Gravity = 0;
            ...
            if (_textboxSimulated)
            {
                _animator.Continue();
                _body.Gravity = -0.15f;
                _body.Velocity = (_storeVelocity != Vector3.Zero) ? _storeVelocity : Vector3.Zero;
                _storeVelocity = Vector3.Zero;
