...
namespace ProjectZ.InGame.GameObjects.MidBoss
{
    internal class MBossGrimCreeperFly : GameObject
    {
        ...
        private MBossGrimCreeper _owner;

        // TODO: sync animations
        public MBossGrimCreeperFly(Map.Map map, MBossGrimCreeper owner, Vector2 position, Vector2 targetPosition) : base(map)
        {
            ...
            _damageState = new AiDamageState(this, _body, _aiComponent, _sprite, _lives, _dropIndex) { OnBurn = OnBurn, SpawnPowerups = false };

            var damageBox = new CBox(EntityPosition, -5, -12, 0, 10, 10, 8, true);
            var hittableBox = new CBox(EntityPosition, -7, -14, 0, 14, 14, 8, true);

            AddComponent(DamageFieldComponent.Index, _damageField = new DamageFieldComponent(damageBox, HitType.Enemy, 2) { IsActive = false, OnDamagedPlayer = OnDamagedPlayer });
