...
namespace ProjectZ.InGame.GameObjects.MidBoss
{
    class MBossGiantBuzzBlob : GameObject
    {
        ...
        public Values.HitCollision OnHit(GameObject gameObject, Vector2 direction, HitType hitType, int damage, bool pieceOfPower)
        {
            ...
            // The player used magic powder on the enemy.
            if (hitType == HitType.MagicPowder && _aiComponent.CurrentStateId == "walk")
            {
                // Do not show the sword message after the player has already
                // figured out that there can be something done with the powder.
                _swordMessage = true;
                _toSlime = true;
                _body.VelocityTarget = Vector2.Zero;
                ...
                return _aiDamageState.OnHit(gameObject, direction, hitType, damage, false);
            }
