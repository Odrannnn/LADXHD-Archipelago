...
namespace ProjectZ.InGame.GameObjects.Bosses
{
    class MBossTurtleRock : GameObject
    {
        ...
        public MBossTurtleRock(Map.Map map, int posX, int posY, string saveKey) : base(map)
        {
            ...
            if (map != null)
                _fieldRectangle = map.GetField((int)_startPosition.X, (int)_startPosition.Y);
            ...
            _aiDamageState = new AiDamageState(this, _body, _aiComponent, _sprite, _lives, 0, false, false, AiDamageState.BlinkTime * 6)
            {
                HitMultiplierX = 0,
                HitMultiplierY = 0,
                ExplosionOffsetY = 16
        ...
        public new void Reset()
        {
            ...
            // use a boolean guard "_initialized" that tracks that the boss is currently actively after playing the ocarina.
            if (_initialized)
                Game1.AudioManager.SetMusicFadeTransition(-1, 2, 350);

            // If the boss is dying, don't let it respawn.
            if (_isDead)
                return;
        ...
        private void Update()
        {
            // Reset the boss if the player leaves the field it's on.
            if (!_playerInField && _initialized)
                Reset();
        }
