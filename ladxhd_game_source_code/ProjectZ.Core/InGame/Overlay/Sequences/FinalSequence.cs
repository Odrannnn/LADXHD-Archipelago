...
namespace ProjectZ.InGame.Overlay.Sequences
{
    class FinalSequence : GameSequence
    {
        ...
        public FinalSequence()
        {
            ...
            // Resolution of the sequence: uses a 16:9 aspect ratio.
            _sequenceWidth = 360;
            _sequenceHeight = 180;
        }
        ...
        public override void Update()
        {
            ...
            // Update the current screen based on the current screen index.
            switch (_screenIndex)
            {
                case 0: { UpdateScreen0(); break; }
                case 1: { UpdateScreen1(); break; }
                case 2: { UpdateScreen2(); break; }
                case 3: { UpdateScreen3(); break; }
                case 4: { UpdateScreen4(); break; }
                case 5: { UpdateScreen5(); break; }
                case 6: { UpdateScreen6(); break; }
            }
            base.Update();
        }
        ...
        private void UpdateScreen1()
        {
            ...
            _s1Seagull0.Position.X += 1 / 6f * Game1.TimeMultiplier;
            _s1Seagull1.Position.X -= 1 / 6f * Game1.TimeMultiplier;
        }
        ...
        private void UpdateScreen3()
        {
            ...
            _s3Seagull.Position.Y -= 1 / 3f * Game1.TimeMultiplier;
        }
        ...
        private void UpdateScreen4()
        {
            ...
            // Whale blocks sunlight darkening the screen.
        ...
        private void UpdateScreen5()
        {
            ...
            // Brighten up the sprite over time.
            var brighness = 155 + (int)(100 * MathHelper.Clamp((_screen5Counter - 750) / 500, 0, 1));
            _s5Link.Color = new Color(brighness, brighness, brighness);

            // Offset the position of the link animation to not cut off the sprite. Round after converting to
            // game pixels, otherwise the offset lands between pixels when the remainder isn't a multiple of scale.
            var posY = MathF.Ceiling((Game1.WindowHeight - _sequenceHeight * _scale) / 2f / _scale);
            if (posY > 0)
                posY = 0;
        ...
        private void UpdateScreen6()
        {
            ...
            if (_finishedCredits)
            {
                // Achievement: The achievement for waking the windfish.
                AchievementManager.Earn(108);

                // If the secret ending was triggered via no deaths or unmissables being set.
                if (_marinEndingTriggered)
                {
                    // Achievement: The Marin ending achievement can only be obtained if not dying.
                    if (_marinEnding)
                        AchievementManager.Earn(109);
        ...
        private void ExitToIntro()
        {
            ...
            Game1.GameManager.ArchipelagoManager.ReportGoal();
        ...
        private bool NextCredits()
        {
            ...
            // When content has been shown, move to the next header.
            if (newContent == null)
            {
                _creditsHeaderIndex++;
                var newHeader = Game1.LanguageManager.GetString("credits_" + _creditsHeaderIndex, null, true);

                // Populate the credits header with the next value.
                if (newHeader != null)
                {
                ...
                // Finished showing all the credits.
                else
                {
                    return false;
                }
            }
            // Populate the credits content with the next value.
            else
            {
        ...
        public override void DrawScaled(SpriteBatch spriteBatch)
        {
            // Don't try to draw the credits text until the final screen index.
            if (_screenIndex != 6)
                return;

            // If the credits header or content currently holds text, draw them centered on the screen.
            if (_creditsHeader != null)
            {
                var textSize = Resources.FontCreditsHeader.MeasureString(_creditsHeader);
                var position = new Vector2(
                    MathF.Round(_sequenceWidth / 2f - textSize.X / 2f),
                    MathF.Round(_cameraPosition.Y + (18 + 48)));
                spriteBatch.DrawString(Resources.FontCreditsHeader, _creditsHeader, position, Color.White);
            }
            if (_creditsContent != null)
            {
                var textSize = Resources.FontCredits.MeasureString(_creditsContent);
                var position = new Vector2(
                    MathF.Round(_sequenceWidth / 2f - textSize.X / 2f),
                    MathF.Round(_cameraPosition.Y + (18 + 70)));
