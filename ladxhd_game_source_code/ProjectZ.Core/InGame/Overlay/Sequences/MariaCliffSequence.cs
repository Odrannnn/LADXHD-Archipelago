...
namespace ProjectZ.InGame.Overlay.Sequences
{
    class MarinCliffSequence : GameSequence
    {
        ...
        public override void OnStart()
        {
            ...
            // If the letter was read in the same session this will be "1".
            string letterRead = Game1.GameManager.SaveManager.GetString("beach_achieve_letter");

            if (!string.IsNullOrEmpty(letterRead))
            {
                // Give the achievement if it was read.
                if (letterRead == "1")
                    AchievementManager.Earn(46);

                Game1.GameManager.SaveManager.RemoveString("beach_achieve_letter");
            }
