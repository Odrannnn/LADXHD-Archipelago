...
namespace ProjectZ.InGame.Overlay.Sequences
{
    ...
    public class SeqSprite : SeqDrawable
    {
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (RoundPosition || GameSettings.PixelSnapping)
            {
            ...
            // The origin adjustment can reintroduce a fraction if an atlas entry has a fractional
            // origin or scale, so snap again after it has been applied.
    ...
    public class GameSequence
    {
        ...
        private void UpdateRenderTarget()
        {
            ...
            // Dispose the old render target so we don't strand it on the GPU causing a memory leak.
        ...
        public virtual void DrawRT(SpriteBatch spriteBatch)
        {
            ...
            // Round the camera position to align with pixels. With Pixel Snapping the camera moves in whole
            // game pixels, which is what lets the grid sit on the render target with no offset at all.
            var cameraX = GameSettings.PixelSnapping
                ? -MathF.Round(_cameraPosition.X)
                : MathF.Round(-_cameraPosition.X * _scale) / _scale;
            var cameraY = GameSettings.PixelSnapping
                ? -MathF.Round(_cameraPosition.Y)
                : MathF.Round(-_cameraPosition.Y * _scale) / _scale;

            var matrix =
                Matrix.CreateTranslation(new Vector3(cameraX, cameraY, 0)) *
        ...
        public virtual void Draw(SpriteBatch spriteBatch, float transparency)
        {
            ...
            // Check if the user enabled the pixel grid and if the resolution can support it.
            var drawGrid = _usePixelGrid && GameSettings.PixelSnapping &&
                           GameSettings.PixelGrid && _scale >= 2 && Resources.PixelGrid != null;

            // If the pixel grid is not being used simply draw the render target.
            if (!drawGrid)
            {
            ...
            // The render target is exactly _sequenceWidth x _sequenceHeight game pixels and the camera inside
            // it is snapped, so a cell is one game pixel and no offset is needed.
            Resources.PixelGrid.Parameters["TextureSize"]?.SetValue(new Vector2(_sequenceWidth, _sequenceHeight));
            ...
            // The blit has to land on a whole screen pixel or the grid drifts away from the content.
            spriteBatch.End();
            spriteBatch.Begin(SpriteSortMode.Deferred, null, SamplerState.PointWrap, null, null, Resources.PixelGrid, Game1.GetMatrix);
            spriteBatch.Draw(_renderTarget, new Vector2(MathF.Round(position.X), MathF.Round(position.Y)), Color.White * transparency);
            spriteBatch.End();
