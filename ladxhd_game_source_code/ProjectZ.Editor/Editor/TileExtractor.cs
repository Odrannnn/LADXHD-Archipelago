...
namespace ProjectZ.Editor
{
    class TileExtractor : Screen
    {
        ...
        private int[,,] TileTexture()
        {
            ...
            // Both lists own their textures and are about to be replaced wholesale.
            foreach (var spr in _sprTiled)
                spr?.Dispose();
            foreach (var spr in _sprTiledRemoved)
                spr?.Dispose();
        ...
        private void LoadOutput()
        {
            ...
            _outputTextureRemoved?.Dispose();
            _outputTextureUntiled?.Dispose();

            _outputTexture = null;
            _outputTextureRemoved = null;
            _outputTextureUntiled = null;
        ...
        private Texture2D RenderTileTexture(GraphicsDevice graphicDevice, List<Texture2D> tiledInput, int tileWidth, int tileHeight, Point tilePadding)
        {
            try
            {
                ...
                var renderTarget = new RenderTarget2D(graphicDevice, sizeWidth, sizeHeight);
        ...
        private void RenderUntiledTexture(GraphicsDevice graphicDevice)
        {
            ...
            var renderTarget = new RenderTarget2D(graphicDevice, width, height);
            ...
            _outputTextureUntiled = renderTarget;
        }
